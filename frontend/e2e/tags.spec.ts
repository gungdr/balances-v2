...
test('tags: create in settings, assign on a position, report route, cleanup', async ({
  page,
}) => {
  ...
  // Open its detail.
  await page.getByRole('row', { name: new RegExp(acctName) }).getByText(acctName).click()
  ...
  const assigned = page.waitForResponse(
    (r) =>
      r.url().includes('/api/tags/assignments') &&
  ...
  await page.reload()
  await expect(page.getByTestId('tag-select').locator('option:checked')).toHaveText(
    tagName,
  )
