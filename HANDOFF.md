...
- **OAuth callback DI refactor**: the only remaining 0% surface in `internal/auth` (`handleCallback`, `googleOAuth.exchange`, `newGoogleOAuth`, `New`, `clearShortCookie`) is the real OAuth wiring. Refactor target: extract an `oauthExchanger interface { exchange(ctx, code) (*googleClaims, error) }`; `Handlers` takes it via constructor; `New` keeps building the real Google-backed one; tests inject a stub. Brings auth from 50.7% to ~85%+ and exercises the founder/invited bootstrap branches end-to-end (currently covered via direct package-private calls in `bootstrap_test.go`). Low risk — the interface boundary already exists implicitly via `googleOAuth.exchange`.
