...
- **Bond snapshot `accrued=0` default + held-to-maturity copy** (next commit after M4.3b-frontend close): for Indonesian govt-primary bonds where coupons pay out directly to the bank account each month, accrued is structurally always 0. Pre-fill the Accrued field with `"0"` in `CreateAccruedInterestSnapshotDialog` with a helper line explaining the override path; add empty-state copy to `BondDetail` noting that held-to-maturity bonds only need one initial snapshot (carry-forward handles the rest). CONTEXT.md should also call out the coupon-disposition distinction (pays_out vs accrues) in the Bond definition. If per-bond override gets fiddly, escalate to a `coupon_disposition` field on `bond_details`.
